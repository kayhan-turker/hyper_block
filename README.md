...
## Development in Hyperbolic Space
...
Below is a 2D analog of hyperbolic space projected in a way that we could visualize on a flat surface. Square edges are straight lines of equal length; however, they shrink and curve further from the centre point, similar to how a map curves and grows toward the poles.
...
But how could a coordinate system exist in this space? If lets say we assumed a square was a room in a house, it wouldn't take long before realizing that what were once floors are now walls, giving an Escher-esque feeling. The edge coloured red below: the person above would save its a floor, but the person below would argue its a wall, even though to get from one person to the other (via the ladders), there is a logical path.
